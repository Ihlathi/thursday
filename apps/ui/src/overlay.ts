...
let visualSettings: VisualSettings = { ...DEFAULT_VISUAL_SETTINGS };
const visualScale = (key: Exclude<keyof VisualSettings, 'reducedMotion'>) => visualSettings[key] / 100;
const animationScale = () => visualScale('animationSpeed');
const reducedMotionEnabled = () => reducedMotion.matches || visualSettings.reducedMotion;

function applyVisualSettings(next: VisualSettings) {
  const previousParticleAmount = visualSettings.borderParticleAmount;
  visualSettings = normalizeVisualSettings(next);
  canvas.classList.toggle('reduced-motion', visualSettings.reducedMotion);
  canvas.style.setProperty('--settle-duration', `${900 / animationScale()}ms`);
  if (previousParticleAmount !== visualSettings.borderParticleAmount && phase === 'settled') {
    clearParticleTimers();
    if (visualSettings.borderParticleAmount === 0) particles = [];
...
function ring(radius: number, birthProgress: number, travelProgress: number) {
  ctx.save();
  const rippleScale = visualScale('rippleIntensity');
  ctx.globalAlpha *= Math.min(1, rippleScale);
  if (rippleScale > 1) ctx.filter = `brightness(${rippleScale})`;
...
function edge(
  ...
  if (impact > 0) {
    const impactRadius = 64 * bloomScale;
    const impactGlow = ctx.createRadialGradient(contactPoint, 0, 0, contactPoint, 0, impactRadius);
    impactGlow.addColorStop(0, `rgba(250,254,255,${Math.min(1, 0.52 * impact * glowScale)})`);
    impactGlow.addColorStop(0.18, `rgba(205,239,255,${Math.min(1, 0.26 * impact * glowScale)})`);
    impactGlow.addColorStop(1, 'rgba(166,218,249,0)');
    ctx.fillStyle = impactGlow;
    ctx.fillRect(contactPoint - impactRadius, 0, impactRadius * 2, impactRadius);
  }

  const bloomDepth = 170 * bloomScale;
  const bloom = ctx.createLinearGradient(0, 0, 0, bloomDepth);
  bloom.addColorStop(0, `rgba(201,235,255,${Math.min(1, 0.3 * contact * glowScale)})`);
  bloom.addColorStop(0.08, `rgba(184,226,251,${Math.min(1, 0.16 * contact * glowScale)})`);
  bloom.addColorStop(0.38, `rgba(174,220,248,${Math.min(1, 0.045 * contact * glowScale)})`);
  bloom.addColorStop(1, 'rgba(180,224,250,0)');
  ctx.save();
  ctx.beginPath();
  ctx.rect(start, 0, end - start, bloomDepth);
  ctx.clip();
  ctx.fillStyle = bloom;
  ctx.fillRect(0, 0, length, bloomDepth);
  ctx.shadowColor = 'rgba(196,234,255,0.8)';
  ctx.shadowBlur = 14 * bloomScale;
  ctx.fillStyle = `rgba(242,251,255,${Math.min(1, 0.88 * contact * glowScale)})`;
  ...
  for (const x of [contactPoint - reach, contactPoint + reach]) {
    ...
    const contactRadius = 42 * bloomScale;
    const glow = ctx.createRadialGradient(x, 0, 0, x, 0, contactRadius);
    glow.addColorStop(0, `rgba(237,250,255,${Math.min(1, 0.3 * contact * glowScale)})`);
    glow.addColorStop(0.25, `rgba(198,234,254,${Math.min(1, 0.14 * contact * glowScale)})`);
    glow.addColorStop(1, 'rgba(190,229,255,0)');
    ctx.fillStyle = glow;
    ctx.fillRect(x - contactRadius, 0, contactRadius * 2, contactRadius);
  }
}
...
function scheduleParticle(side: number, initial = false) {
  ...
  const amount = visualScale('borderParticleAmount');
  if (phase !== 'settled' || amount <= 0) return;
  ...
  const baseDelay = initial
    ...
    : edgeCount === 0
      ...
      : quietMoment
        ? 650 + Math.random() * 450
        : 100 + Math.random() * 150;
  const nextDelay = baseDelay / Math.max(0.18, amount);
  particleTimers[side] = window.setTimeout(() => {
    if (phase !== 'settled') return;
    const currentEdgeCount = particles.filter((particle) => particle.side === side).length;
    if (currentEdgeCount >= Math.max(2, Math.round(12 * amount))) return scheduleParticle(side);
    const along = 0.08 + Math.random() * 0.84;
    const inward = (18 + Math.random() * 28) * visualScale('borderParticleTravelDistance');
...
function moveJarvisCursor(to: SummonOrigin, from = cursorPosition, onComplete?: () => void) {
  ...
  const curve = Math.max(2, Math.min(58, distance * curvature * visualScale('pathPersonality')))
    * (Math.random() < 0.5 ? -1 : 1);
  let firstBend = 0.72;
  let secondBend = 0.56;
  ...
  cursorMotion = {
    ...
    ) / (visualScale('movementSpeed') * animationScale()),
    style,
    onComplete,
  };
...
function drawTrail(now: number) {
  const trailLifetime = CURSOR_TUNING.trailLifetimeMs * visualScale('trailLifetime');
  trail = trail.filter((point) => now - point.born < trailLifetime);
  const trailScale = visualScale('trailIntensity');
  ctx.save();
  ctx.globalAlpha *= Math.min(1, trailScale);
  if (trailScale > 1) ctx.filter = `brightness(${trailScale})`;
  for (let index = 1; index < trail.length; index += 1) {
    const previous = trail[index - 1];
    const point = trail[index];
    const age = clamp((now - point.born) / trailLifetime);
...
function drawClickBorderImpactPoint(side: number, along: number, strength: number) {
  strength *= visualScale('borderImpactIntensity');
...
function drawClickSequence(now: number) {
  ...
  const clickTimingScale = visualScale('clickRippleSpeed') * animationScale();
  const clickSettleMs = CURSOR_TUNING.clickSettleMs / clickTimingScale;
  const settle = clamp(elapsed / clickSettleMs);
  if (elapsed >= clickSettleMs && !clickSequence.fired) {
    clickSequence.fired = true;
    clickSequence.action?.();
  }
  const waveElapsed = elapsed - clickSettleMs;
  if (waveElapsed >= 0) {
    ...
    const clickSpeed = CURSOR_TUNING.clickWaveSpeed * clickTimingScale;
    const clickIntensity = visualScale('clickRippleIntensity');
    const waveRadius = waveElapsed * clickSpeed / 1000;
    const waveDuration = maximumDistance / clickSpeed * 1000;
    ...
    if (localProgress < 1) {
      ...
      const ringAlpha = alpha * CURSOR_TUNING.clickLocalRingIntensity * clickIntensity;
    ...
    if (waveRadius > 1 && waveRadius < maximumDistance + CURSOR_TUNING.clickWaveWidth) {
      ...
      sheet.addColorStop(Math.max(0, crest - 0.62), `rgba(163,219,250,${0.008 * waveEnvelope * clickIntensity})`);
      sheet.addColorStop(Math.max(0, crest - 0.34), `rgba(176,227,252,${CURSOR_TUNING.clickWaveWakeIntensity * waveEnvelope * clickIntensity})`);
      sheet.addColorStop(Math.max(0, crest - 0.12), `rgba(202,239,255,${CURSOR_TUNING.clickWaveIntensity * 0.46 * waveEnvelope * clickIntensity})`);
      sheet.addColorStop(crest, `rgba(237,250,255,${CURSOR_TUNING.clickWaveIntensity * waveEnvelope * clickIntensity})`);
      sheet.addColorStop(Math.min(1, crest + 0.1), `rgba(191,232,253,${CURSOR_TUNING.clickWaveIntensity * 0.2 * waveEnvelope * clickIntensity})`);
    ...
    for (const point of clickField) {
      ...
      const arrival = distance / clickSpeed * 1000;
      ...
      const alpha = illumination * attenuation * point.reflectivity * CURSOR_TUNING.clickFieldIntensity
        * visualScale('screenFieldIntensity') * clickIntensity;
    ...
    const flash = Math.min(1, Math.sin(flashProgress * Math.PI)
      * (1 - smoothstep(flashProgress) * 0.35)
      * CURSOR_TUNING.clickLocalIntensity * clickIntensity);
    if (flash > 0) {
...
function drawCursorAura(now: number, focus: number) {
  ...
  const brightness = visualScale('auraBrightness');
  const feathering = visualScale('auraFeathering');
  const blueBalance = visualScale('auraBlueBalance');
  ctx.save();
  ctx.globalAlpha *= Math.min(1, brightness);
  ctx.filter = `brightness(${Math.max(1, brightness)}) saturate(${0.58 + blueBalance * 0.52})`;
  const breathe = 1 + Math.sin((now / CURSOR_TUNING.auraBreathingMs) * Math.PI * 2)
    * CURSOR_TUNING.auraBreathingStrength;
  const radius = CURSOR_TUNING.auraGlowRadius * visualScale('auraSize')
    * (0.86 + feathering * 0.14) * breathe * (1 - focus * 0.18);
  const auraFalloff = clamp(CURSOR_TUNING.auraFalloff - (feathering - 1) * 0.13);
  ...
  outer.addColorStop(auraFalloff, `rgba(139,201,246,${CURSOR_TUNING.auraOuterBloomIntensity * 0.085})`);
  ...
  const coreRadius = CURSOR_TUNING.auraCoreRadius * visualScale('auraSize') * breathe;
...
function spawnCursorParticle(now: number) {
  const amount = visualScale('cursorParticleAmount');
  if (amount <= 0 || cursorParticles.length >= Math.max(1, Math.round(CURSOR_TUNING.cursorParticleMaxCount * amount))) return;
  const angle = Math.random() * Math.PI * 2;
  const spawnRadius = 7 + Math.random() * 14;
  const travel = (17 + Math.random() * 24) * visualScale('cursorParticleTravelDistance');
...
function drawCursorParticles(now: number) {
  ...
  const amount = visualScale('cursorParticleAmount');
  if (emitting && amount > 0 && now >= nextCursorParticleAt) {
    spawnCursorParticle(now);
    const delayRange = CURSOR_TUNING.cursorParticleMaxDelayMs - CURSOR_TUNING.cursorParticleMinDelayMs;
    nextCursorParticleAt = now
      + (CURSOR_TUNING.cursorParticleMinDelayMs + Math.random() * delayRange) / Math.max(0.18, amount);
  } else if (!emitting) {
...
function drawBorder(radius: number, distances: number[], origin: SummonOrigin = summonOrigin, now = performance.now()) {
  const { x: originX, y: originY } = origin;
  const breathe = phase === 'settled'
    ? 1 + Math.sin(now / 1250) * 0.055 * visualScale('borderBreathingIntensity')
    : 1;
  const glowScale = visualScale('borderGlowIntensity') * breathe;
  const bloomScale = visualScale('borderBloomAmount');
  edge(width, originY, originX, radius, glowScale, bloomScale);
  ctx.save(); ctx.translate(width, height); ctx.rotate(Math.PI);
  edge(width, height - originY, width - originX, radius, glowScale, bloomScale); ctx.restore();
  ctx.save(); ctx.translate(0, height); ctx.rotate(-Math.PI / 2);
  edge(height, originX, height - originY, radius, glowScale, bloomScale); ctx.restore();
  ctx.save(); ctx.translate(width, 0); ctx.rotate(Math.PI / 2);
  edge(height, width - originX, originY, radius, glowScale, bloomScale); ctx.restore();
  const corners = [[0, 0], [width, 0], [0, height], [width, height]];
  corners.forEach(([x, y], index) => {
    const corner = clamp((radius - distances[index] + 30) / 32);
    const cornerRadius = 170 * bloomScale;
    const glow = ctx.createRadialGradient(x, y, 0, x, y, cornerRadius);
    glow.addColorStop(0, `rgba(220,241,255,${Math.min(1, 0.19 * corner * glowScale)})`);
    glow.addColorStop(0.3, `rgba(185,226,250,${Math.min(1, 0.065 * corner * glowScale)})`);
...
function draw(now: number) {
  ...
  const overallIntensity = visualScale('visualIntensity');
  ctx.globalAlpha = Math.min(1, overallIntensity);
  ctx.filter = overallIntensity > 1 ? `brightness(${overallIntensity})` : 'none';
  ...
  if (phase === 'summoning') {
    ...
    const summonSpeed = animationScale() * visualScale('summonAnimationSpeed');
    const adjustedBirthDuration = birthDuration / summonSpeed;
    const adjustedTravelDuration = travelDuration / summonSpeed;
    const birthProgress = reducedMotionEnabled() ? 1 : clamp(elapsed / adjustedBirthDuration);
    const travelProgress = reducedMotionEnabled()
      ? 1
      : clamp((elapsed - adjustedBirthDuration) / adjustedTravelDuration);
    ...
    drawBorder(currentRadius, distances, lifecycleOrigin, now);
    drawCursorSystem(now);
    if (travelProgress < 1) {
      queueDraw();
    } else {
  ...
  const adjustedDismissDuration = dismissDuration
    / (animationScale() * visualScale('dismissAnimationSpeed'));
  const dismissProgress = reducedMotionEnabled()
    ? 1
    : clamp((now - dismissStarted) / adjustedDismissDuration);
...
applyVisualSettings(await loadVisualSettings());
resize();
